...
import { Container, Content, ContestArea, ResultContestArea } from "./styles";
import { ContestType } from "@/types/contestTypes" ;

type CurrentStateType = ContestType;
...
type ContestDataListTypes = {
  loteriaId : number;
  concursoId : string;
  dataConcurso: Date;
  resultadoConcurso : number[];
...
export function MainPage() {
  ...
  const [currentContest , setCurrentContest] = useState<CurrentStateType>("mega-sena");
  const [contestDataList , setContestDataList] = useState<ContestDataListTypes[]>()
  ...
  async function getDataFromApi() {
    ...
    const tempData  =  response.data.map(async (item:ApiResponseType)=>{

      const contestResult =  await api.get(`/concursos/${item.concursoId}`)

      return {

        loteriaId : item.loteriaId,
        concursoId: item.concursoId,
        dataConcurso: contestResult.data.data,
        resultadoConcurso : contestResult.data.numeros 

      }
    }) as ContestDataListTypes[] ;

    
    Promise.all(tempData).then( res => setContestDataList(res));
  ...
  function formatContestTitle(contest:string){
  ...
  function getContestId(contest:string){
  ...
  return (
    <Container>
      <Content>
        ...
        <ContestArea contestStyle={currentContest}>
            ...
            <div className="title-contest-area">
              ...
              <p>{ formatContestTitle(currentContest) }</p>
            </div>


            <div className="contest-info-area">
              <p className="contest-info-title">Concurso</p>
              <p className="contest-info-text">
                  
                {
                
                contestDataList?.map( item => {

                  if(item.loteriaId === getContestId(currentContest) )
                  return (
                    <p className="acaba"> {item.concursoId} - {new Intl.DateTimeFormat('pt-BR').format(new Date(item.dataConcurso))} </p>
                  )
                })
                }
        ...
        <ResultContestArea>
          ...
          <div className="result-field">
            {contestDataList?.map( item => {
                if(item.loteriaId === getContestId(currentContest) ) {
                  return (
                    item.resultadoConcurso.map(number => {
                      return (
                        <div className="contest-number-result">{number}</div>
                      )
          ...
          <p className="footer">Este sorteio é meramente ilustrativo e não possui nenhuma ligação com a CAIXA.</p>
